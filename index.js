...
import { useEffect, useCallback, createElement, Fragment, useMemo } from 'react';
...
export function useFillerRows(hooks)
{
...
export function useWheelPaging(hooks)
{
...
export function useSelectSingleRow(hooks)
{
  function getRowProps(props, {row, instance})
  {
    const { setSelectedRowIndex, state: {selectedRowIndex} } = instance;
    console.log(selectedRowIndex);
    const onClick = () => { setSelectedRowIndex(row.index); console.log(`Selecting ${row.index}`); };
  ...
  function useInstance(instance)
  {
    const setSelectedRowIndex = (index) => instance.dispatch(
      {type: actions.setSelectedRowIndex, index}
    );
    const selectPreviousRow = () => instance.dispatch(
      {type: actions.moveSelection, delta: -1}
    );
    const selectNextRow = () => instance.dispatch(
      {type: actions.moveSelection, delta: 1}
    );
    const pageUp = () => instance.dispatch(
      {type: actions.moveSelection, delta: -instance.state.pageSize}
    );
    const pageDown = () => instance.dispatch(
      {type: actions.moveSelection, delta: instance.state.pageSize}
    );
    useEffect(
  ...
  function reducer(state, action, previousState, instance)
  {
    ...
    if (action.type === actions.setSelectedRowIndex) {
      const position_in_page = instance.page.findIndex(row => row.index == action.index);
    ...
    if (action.type === actions.moveSelection) {
      const selectedRowPosition = Math.max(0, Math.min(instance.rows.length - 1, state.selectedRowPosition + action.delta));
    ...
    if (action.type === actions.updateRowPosition) {
      const selectedRowPosition = instance.rows.findIndex(row => row.index == state.selectedRowIndex);
      const pageIndex = Math.floor(selectedRowPosition / state.pageSize);
...
export function TableBodyRows({page, prepareRow}) {
  return e(Fragment, null, page.map(row => {
    prepareRow(row);
...
export function TableHead({headerGroups, makecell}) {
  return e('thead', null, headerGroups.map(headerGroup => (
    e('tr', {...headerGroup.getHeaderGroupProps()},
...
export function SelectColumnFilter({
  ...
  const [sortedOptions, optionToIndex] = useMemo(() => {
    ...
    sortedOptions.forEach((option, idx) => { optionToIndex.set(option, idx) });
