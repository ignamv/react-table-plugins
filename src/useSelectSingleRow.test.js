...
const { setupReactTest, teardownReactTest, getContainer, ReactTableTestHarness } = require("./testHelpers");
...
test('useSelectSingleRow', () => {
    ...
    act(() => { getLastRender().instance.nextPage(); });
    sendkey('ArrowUp');
    assertSelected(8, 8);
    // Selection stays visible after sorting
    act(() => { getLastRender().instance.setSortBy([{id: 'col0'}])});
    let newPosition = getLastRender().instance.rows.findIndex(r => r.index == 8);
    ...
    act(() => { getLastRender().instance.setFilter('col1', testData[8].col1); });
    ...
    act(() => { getLastRender().instance.setFilter('col0', newFilter); });
